  ...
  return (
    <Shell>
      <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div>
          <h1 className="text-4xl font-display font-bold mb-2">Overview</h1>
          <p className="text-muted-foreground text-lg">Your incubator's key metrics at a glance.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <Card className="p-6 bg-white border-gray-200 shadow-lg hover-elevate card-3d">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-sm font-medium text-muted-foreground mb-1">Total Startups</p>
                <h3 className="text-3xl font-display font-bold">{startups?.length || 0}</h3>
              </div>
          ...
          <Card className="p-6 bg-white border-gray-200 shadow-lg hover-elevate card-3d">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-3xl font-display font-bold">{applications?.length || 0}</h3>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <Card className="p-6 border-gray-200 bg-white shadow-lg hover-elevate">
            <h3 className="text-lg font-semibold mb-6">Application Pipeline</h3>
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={pipelineData}>
                  ...
                  />
                  <Bar dataKey="value" fill="#015185" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </Card>
          ...
          <Card className="p-6 border-gray-200 bg-white shadow-lg hover-elevate">
            <h3 className="text-lg font-semibold mb-6">Funding Trajectory</h3>
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={fundingData.length ? fundingData : [{name: 'Jan', amount: 100000}, {name: 'Feb', amount: 250000}]}>
                  ...
                  <Tooltip 
                    contentStyle={{ backgroundColor: '#fff', border: '1px solid #e5e7eb', borderRadius: '8px' }}
                  />
                  <Line type="monotone" dataKey="amount" stroke="#015185" strokeWidth={3} dot={{r: 4, fill: '#015185'}} activeDot={{r: 6}} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </Card>
